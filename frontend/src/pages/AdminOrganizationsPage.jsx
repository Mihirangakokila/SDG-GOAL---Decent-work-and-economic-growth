...
export default function AdminOrganizationsPage() {
  ...
  const [reportOrgCategory, setReportOrgCategory] = useState('all')
  ...
  const youthTotalPages = Math.max(1, Math.ceil(youthProfiles.length / youthLimit) || 1)
  const youthPaged = useMemo(() => {
    const start = (youthPage - 1) * youthLimit
    return youthProfiles.slice(start, start + youthLimit)
  }, [youthProfiles, youthPage, youthLimit])
  ...
  const handleDownloadReport = () => {
    ...
    try {
      const youthInRange = youthProfiles.filter((p) => inDateRange(p.createdAt, fromDate, toDate))
      const orgsInRangeAll = allOrganizations.filter((o) => inDateRange(o.createdAt, fromDate, toDate))
      const orgsInRange = orgsInRangeAll.filter((o) => {
        if (reportOrgCategory === 'all') return true
        if (reportOrgCategory.startsWith('industry:')) {
          const industry = reportOrgCategory.slice('industry:'.length)
          return (o.industry || '').trim() === industry
        }
        return true
      })
      ...
      const reportTitle = isYouthReport
        ...
        : `InternHub Admin Dashboard Report (Organization Profiles${
            reportOrgCategory === 'all' ? '' : ` - ${reportOrgCategory.replace('industry:', 'Industry: ')}`
          })`
      ...
      doc.save(
        `${
          isYouthReport
            ? 'youth'
            : `organization-${reportOrgCategory.replace(':', '-').replace(/\s+/g, '-').toLowerCase()}`
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="flex flex-col lg:flex-row gap-4 lg:items-end">
          ...
          {adminTab === 'organizations' ? (
            <div className="w-full lg:w-72">
              <label className="label">Organization report category</label>
              <select
                className="input"
                value={reportOrgCategory}
                onChange={(e) => setReportOrgCategory(e.target.value)}
              >
                <option value="all">All organizations</option>
                {industryOptions.map((ind) => (
                  <option key={`report-${ind}`} value={`industry:${ind}`}>
                    Industry: {ind}
                  </option>
                ))}
              </select>
      ...
      {adminTab === 'youth' ? (
        <>
          <AdminYouthCharts profiles={youthProfiles} />
          ...
          <div className="card overflow-hidden">
            {youthLoading ? (
              ...
              <div className="text-center py-16 text-slate-500 text-sm">No youth profiles found.</div>
            ) : (
            ...
            {!youthLoading && youthProfiles.length > 0 ? (
              <div className="flex items-center justify-between px-6 py-4 border-t border-slate-100">
                <p className="text-xs text-slate-400">
                  Page {youthPage} of {youthTotalPages} · {youthProfiles.length} profile
                  {youthProfiles.length !== 1 ? 's' : ''}
                </p>
